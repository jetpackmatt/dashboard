...
export function CreditsTable({
  ...
  const handleSortChange = React.useCallback((field: string, direction: 'asc' | 'desc') => {
    ...
    setPageIndex(0)
    const colConfig = CREDITS_TABLE_CONFIG.columns.find(c => c.id === field)
  ...
  const handleExport = React.useCallback(async (options: { format: ExportFormat; scope: ExportScope }) => {
    ...
    if (scope === 'current') {
      exportData(data as unknown as Record<string, unknown>[], {
        format: exportFormat, scope, filename: 'credits',
        ...toExportMapping(CREDITS_INVOICE_COLUMNS),
      })
    } else {
      const params = new URLSearchParams()
      if (clientId) params.set('clientId', clientId)
      ...
      startClientExport({
        apiUrl: '/api/data/billing/credits',
        params,
        source: 'Credits',
        totalCount,
  ...
  }, [data, clientId, dateRange, effectiveReasonFilter, searchQuery, totalCount, startClientExport])
