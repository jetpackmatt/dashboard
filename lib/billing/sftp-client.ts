...
/**
 * Populate refund transaction breakdown from matching charges
 *
 * Refund transactions (transaction_type = 'Refund') don't appear in SFTP files.
 * For proper markup calculation, we need to copy breakdown values from the
 * corresponding Charge transaction and negate them.
 *
 * Pattern: When a shipment is rebilled, ShipBob creates:
 * - A Refund transaction (reversing the original charge)
 * - A new Charge transaction (with potentially different amounts)
 *
 * This function finds refunds with null base_cost and copies breakdown from
 * the matching charge in the same invoice.
 */
export async function populateRefundBreakdowns(
  supabase: SupabaseClient
  ...
  // Step 1: Find all Refund transactions with null base_cost
  const { data: refunds, error: refundError } = await supabase
    .from('transactions')
    .select('id, reference_id, invoice_id_sb, cost')
    .eq('reference_type', 'Shipment')
    .eq('fee_type', 'Shipping')
  ...
  if (refundError) {
    result.errors.push(`Error fetching refunds: ${refundError.message}`)
    return result
  }

  if (!refunds || refunds.length === 0) {
    console.log('  No refunds needing breakdown update')
    return result
  }

  console.log(`  Found ${refunds.length} refunds needing breakdown data`)

  // Step 2: For each refund, find matching charge in same invoice
  for (const refund of refunds) {
    // Find the corresponding Charge transaction in the same invoice for same shipment
    // It should have the same shipment_id, same invoice_id_sb, but positive cost
    const { data: charges, error: chargeError } = await supabase
      .from('transactions')
      .select('id, base_cost, surcharge, insurance_cost, cost')
      .eq('reference_type', 'Shipment')
      .eq('fee_type', 'Shipping')
      .eq('reference_id', refund.reference_id)
      .eq('invoice_id_sb', refund.invoice_id_sb)
      .neq('transaction_type', 'Refund')
      .not('base_cost', 'is', null)
      .limit(1)

    if (chargeError) {
      result.errors.push(`Error finding charge for refund ${refund.id}: ${chargeError.message}`)
      continue
    }

    if (!charges || charges.length === 0) {
      // No matching charge in same invoice - try to find ANY charge for this shipment
      // (The original charge might be in a different invoice)
      const { data: anyCharge, error: anyChargeError } = await supabase
        .from('transactions')
        .select('id, base_cost, surcharge, insurance_cost, cost')
        .eq('reference_type', 'Shipment')
        .eq('fee_type', 'Shipping')
      ...
      if (anyChargeError || !anyCharge || anyCharge.length === 0) {
        // Still no charge found - derive from refund's cost (fallback)
        // Use the absolute value of the refund cost as base, no surcharge
        const absBaseCost = Math.abs(Number(refund.cost) || 0)

        const { error: updateError } = await supabase
          .from('transactions')
          .update({
            base_cost: -absBaseCost,
            surcharge: 0,
            insurance_cost: 0
          })
          .eq('id', refund.id)
        ...
        if (updateError) {
          result.errors.push(`Error updating refund ${refund.id}: ${updateError.message}`)
        } else {
          result.updated++
        }
        continue
      ...
      // Use the charge from another invoice
      const charge = anyCharge[0]
      const { error: updateError } = await supabase
        .from('transactions')
        .update({
          base_cost: -(Number(charge.base_cost) || 0),
          surcharge: -(Number(charge.surcharge) || 0),
          insurance_cost: -(Number(charge.insurance_cost) || 0)
        })
        .eq('id', refund.id)
    ...
    // Found matching charge in same invoice - negate its breakdown values
  ...
  console.log(`  Refund breakdown update complete: ${result.updated} updated, ${result.errors.length} errors`)
