...
export async function runPreflightValidation(
  ...
  // SFTP cost files are generated by ShipBob overnight — same-day and previous-day
  // shipments physically don't have SFTP data yet (the file for day N drops on
  // day N+1 morning). Exclude them from BOTH numerator AND denominator so
  // preflight doesn't false-positive on freshly-shipped transactions.
  const sftpCutoff = new Date()
  sftpCutoff.setUTCHours(0, 0, 0, 0)
  sftpCutoff.setUTCDate(sftpCutoff.getUTCDate() - 1)
  const sftpEligibleTx = shippingTransactions.filter(tx => !tx.charge_date || new Date(tx.charge_date as string) < sftpCutoff)
  const sftpEligibleWithBase = sftpEligibleTx.filter(tx => tx.base_cost !== null).length
  checkField('SFTP_BREAKDOWN', 'base_cost (SFTP breakdown)', sftpEligibleTx.length, sftpEligibleWithBase, 'shipments')
